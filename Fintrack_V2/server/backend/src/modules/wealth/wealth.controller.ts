...
export class WealthController {
  ...
  @Get('summary')
  @ApiOperation({ summary: 'Get wealth summary: assets, liabilities, net worth' })
  ...
  async addAsset(@CurrentUser() user: RequestUser, @Body() body: Record<string, unknown>) {
    return this.wealthService.addAsset(user.sub, body);
  }

  @Post('liabilities')
  @ApiOperation({ summary: 'Add a new liability' })
  async addLiability(@CurrentUser() user: RequestUser, @Body() body: Record<string, unknown>) {
