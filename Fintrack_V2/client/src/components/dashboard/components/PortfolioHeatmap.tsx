...
import { C, MONO, SYNE, SANS, fmtShort } from '../../../utils/dashboard/tokens';
import { Shard }             from './Shard';

function cellBg(pct: number, kind: string): string {
  if (kind === 'ASSET') return 'rgba(108,116,255,0.1)';
  if (pct > 15)  return 'rgba(13,220,155,0.6)';
  if (pct > 5)   return 'rgba(13,220,155,0.35)';
    ...
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.4 }}>
      <Shard style={{ padding: '18px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div>
            <p style={{ fontFamily: SYNE, fontSize: 13, fontWeight: 700, color: C.text0, marginBottom: 2 }}>Portfolio Heatmap</p>
            <p style={{ fontSize: 10, color: C.text2, fontFamily: SANS }}>Size = value · Color = P&L</p>
          </div>
          <div style={{ display: 'flex', gap: 10, fontSize: 9 }}>
            {[{ c: C.jade, l: 'Gain' }, { c: C.terra, l: 'Loss' }, { c: C.indigo, l: 'Asset' }].map(s => (
              <span key={s.l} style={{ display: 'flex', alignItems: 'center', gap: 3, color: C.text2, fontFamily: SANS }}>
        ...
        {!items.length ? (
          <div style={{ height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 6 }}>
            ...
            <span style={{ fontSize: 12, color: C.text1, fontFamily: SANS }}>Add investments to see heatmap</span>
          </div>
        ) : (
            ...
            {items.map((h, i) => {
              ...
              return (
                <motion.div key={h.name + i}
                  ...
                  style={{ borderRadius: 8, padding: '9px 11px', minHeight: w > 18 ? 90 : w > 8 ? 70 : 56, flexBasis: w > 18 ? '43%' : w > 8 ? '28%' : w > 4 ? '20%' : '16%', flexGrow: 1, background: cellBg(h.pct, h.kind), border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'hidden', position: 'relative', cursor: 'default' }}>
                  ...
                  <p style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.88)', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', lineHeight: 1.3, marginBottom: 6, fontFamily: SANS }}>{h.name}</p>
