...
import {
  Plus, X, MoreVertical, Pencil, Trash2, Target,
  TrendingUp, AlertCircle, CheckCircle2, Clock,
  Search, ArrowUpCircle, Flame, Trophy,
} from "lucide-react";
...
function ProgressModal({
  goal, onClose, onSubmit, isPending,
}: {
  goal: Goal;
  onClose: () => void;
  ...
  const pct = goal.target > 0 ? Math.min(Math.round((amount / goal.target) * 100), 100) : 0;
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900 dark:text-white leading-tight">{goal.title}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Update saved amount</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="mb-4">
            ...
            <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
              <motion.div
                ...
                style={{ height: "100%", borderRadius: 999, background: goal.color }}
              />
            </div>
...
function GoalCard({
  ...
  const stripColor = isComplete ? "#10b981" : isOverdue ? "#ef4444" : isAtRisk ? "#f59e0b" : g.color;
      ...
      <div style={{ padding: "20px 20px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <h3
              style={{
                fontSize: 15, fontWeight: 700, lineHeight: 1.3, marginBottom: 4,
              ...
              <AnimatePresence>
                {menuOpen && (
                  <>
                    ...
                    >
                      {[
                        { icon: ArrowUpCircle, label: "Update Progress", color: "#6366f1", onClick: () => { onUpdateProgress(g); setMenuOpen(false); } },
                        { icon: Pencil, label: "Edit Goal", color: undefined, onClick: () => { onEdit(g); setMenuOpen(false); } },
                      ].map(item => (
                        <button
                          key={item.label}
                          ...
                          className={item.color ? "" : "text-gray-700 dark:text-gray-200"}
                      ...
                      <div style={{ height: 1, margin: "4px 0" }} className="bg-gray-100 dark:bg-gray-700" />
                      <button
                        onClick={() => { onDelete(g.id); setMenuOpen(false); }}
                        style={{
                          width: "100%", padding: "8px 14px",
                          display: "flex", alignItems: "center", gap: 8,
                          fontSize: 13, background: "transparent",
                          border: "none", cursor: "pointer", textAlign: "left",
                          color: "#ef4444", transition: "background 0.1s",
                        }}
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <div>
              <p style={{ fontSize: 10, marginBottom: 1 }} className="text-gray-400">Saved</p>
              <p style={{ fontSize: 13, fontWeight: 700 }} className="text-gray-900 dark:text-white">
                {fmtCurrency(g.current)}
              </p>
            </div>
            <div style={{ textAlign: "right" }}>
              <p style={{ fontSize: 10, marginBottom: 1 }} className="text-gray-400">Target</p>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                border: "none", cursor: "pointer", color: g.color,
                padding: "2px 0",
              }}
...
export default function GoalsPage() {
  ...
  const updateGoal = useMutation({
    mutationFn: async (dto: any) => (await api.patch(`/goals/${selectedGoal?.id}`, dto)).data,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["goals"] }); closeModal(); },
  });
  ...
  const updateProgress = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["goals"] }); setProgressGoal(null); },
  });

  /* ── Stats ── */
  const stats = useMemo(() => {
    if (!goals.length) return { totalTarget: 0, totalCurrent: 0, completed: 0, inProgress: 0 };
    return {
      ...
      inProgress:   goals.filter(g => g.current < g.target && calcDaysLeft(g.deadline) > 0).length,
    };
  }, [goals]);
  ...
  const displayed = useMemo(() => {
    ...
    if (filterKey === "completed") list = list.filter(g => g.current >= g.target);
    else if (filterKey === "on-track") list = list.filter(g => calcDaysLeft(g.deadline) > 30 && g.current < g.target);
    ...
    list.sort((a, b) => {
      ...
      if (sortKey === "progress") return (b.current / b.target) - (a.current / a.target);
  ...
  const closeModal = () => { setModalMode("closed"); setSelectedGoal(null); reset(); };
  ...
  return (
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 20px" }}>
      ...
      {!isLoading && goals.length > 0 && (
        ...
        >
          {[
            { icon: Target,    iconColor: "#6366f1", bg: "rgba(99,102,241,0.1)",  label: "Total Target",  val: fmtCurrency(stats.totalTarget)  },
            { icon: TrendingUp,iconColor: "#10b981", bg: "rgba(16,185,129,0.1)",  label: "Total Saved",   val: fmtCurrency(stats.totalCurrent) },
            { icon: Trophy,    iconColor: "#f59e0b", bg: "rgba(245,158,11,0.1)",  label: "Completed",     val: `${stats.completed} goal${stats.completed !== 1 ? "s" : ""}` },
            { icon: Flame,     iconColor: "#ef4444", bg: "rgba(239,68,68,0.1)",   label: "In Progress",   val: `${stats.inProgress} active`     },
          ].map(s => (
            <div
              key={s.label}
              style={{ padding: "14px 16px", borderRadius: 16, display: "flex", alignItems: "center", gap: 12 }}
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: 11, marginBottom: 2 }} className="text-gray-500 dark:text-gray-400">{s.label}</p>
                <p style={{ fontSize: 14, fontWeight: 700 }} className="text-gray-900 dark:text-white truncate">{s.val}</p>
              </div>
            </div>
          ))}
        </motion.div>
      ...
      {!isLoading && goals.length > 0 && (
        <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {/* Filter tabs */}
          <div style={{ display: "flex", gap: 4, padding: 4, borderRadius: 12, flexShrink: 0 }} className="bg-gray-100 dark:bg-gray-800">
            {FILTERS.map(f => (
              <button
                key={f.key}
          ...
          <select
            ...
            style={{ padding: "8px 12px", fontSize: 12, borderRadius: 10, border: "1px solid", background: "transparent", cursor: "pointer", outline: "none", flexShrink: 0 }}
      ...
      {isLoading ? (
        ...
        <div
          style={{
            ...
            border: "2px dashed", borderRadius: 24,
            display: "flex", flexDirection: "column", alignItems: "center", gap: 12,
          }}
          className="border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/30"
        >
          <div style={{ width: 60, height: 60, borderRadius: 18, background: "rgba(99,102,241,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Target size={26} color="#6366f1" />
          </div>
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 4 }} className="text-gray-900 dark:text-white">No goals yet</h3>
            <p style={{ fontSize: 13 }} className="text-gray-500 dark:text-gray-400">Set financial milestones and track your progress.</p>
          </div>
          <button
            onClick={openCreate}
            style={{ fontSize: 13, fontWeight: 600, color: "#6366f1", background: "none", border: "none", cursor: "pointer", padding: 0 }}
          >
            Create your first goal →
          </button>
        </div>
      ) : displayed.length === 0 ? (
        <div style={{ padding: 48, textAlign: "center", fontSize: 14 }} className="text-gray-500 dark:text-gray-400">
      ...
      <AnimatePresence>
        {modalMode !== "closed" && (
          <div style={{ position: "fixed", inset: 0, zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            ...
            >
              {/* Colored top bar */}
              <div style={{ height: 3, background: `linear-gradient(90deg, ${watchedColor}, ${watchedColor}99)` }} />

              <div style={{ padding: "20px 22px 22px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                  <h2 style={{ fontSize: 16, fontWeight: 700 }} className="text-gray-900 dark:text-white">
                    {modalMode === "create" ? "New Goal" : "Edit Goal"}
                  </h2>
                  <button onClick={closeModal} style={{ width: 28, height: 28, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", border: "none", cursor: "pointer", background: "transparent" }}
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    ...
                    {/* Title */}
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}
                        className="text-gray-500 dark:text-gray-400">Goal Name</label>
                      <input
                        {...register("title", { required: true })}
                        placeholder="e.g. Emergency Fund"
                        style={{ width: "100%", padding: "9px 12px", borderRadius: 10, border: "1px solid", fontSize: 13, outline: "none", background: "transparent" }}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}
                          className="text-gray-500 dark:text-gray-400">Target ₹</label>
                        <input
                      ...
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}
                          className="text-gray-500 dark:text-gray-400">Deadline</label>
                        <input
                          {...register("deadline", { required: true })}
                          type="date"
                          style={{ width: "100%", padding: "9px 10px", borderRadius: 10, border: "1px solid", fontSize: 13, outline: "none", background: "transparent" }}
                    ...
                    {/* Icon picker */}
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}
                        className="text-gray-500 dark:text-gray-400">Icon</label>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {PRESET_ICONS.map(icon => (
                          <button
                            ...
                            style={{
                              width: 38, height: 38, borderRadius: 10, fontSize: 17, border: "1.5px solid",
                              display: "flex", alignItems: "center", justifyContent: "center",
                              cursor: "pointer", transition: "all 0.15s",
                              background: watchedIcon === icon ? `${watchedColor}18` : "transparent",
                              borderColor: watchedIcon === icon ? watchedColor : "transparent",
                              transform: watchedIcon === icon ? "scale(1.1)" : "scale(1)",
                            }}
                            className={watchedIcon === icon ? "" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"}
                          >
                            {icon}
                          </button>
                        ))}
                    ...
                    <div>
                      <label style={{ fontSize: 11, fontWeight: 600, display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}
                        className="text-gray-500 dark:text-gray-400">Color</label>
                        ...
                        {PRESET_COLORS.map(color => (
                          <button
                            ...
                            style={{
                              ...
                              transform: watchedColor === color ? "scale(1.2)" : "scale(1)",
                              boxShadow: watchedColor === color ? `0 0 0 2px white, 0 0 0 4px ${color}` : "none",
                            }}
                          />
                        ))}
                      </div>
                    ...
                    {/* Preview */}
                    <div style={{ padding: "12px 14px", borderRadius: 12, border: "1px solid", display: "flex", alignItems: "center", gap: 10 }}
                      style2={{ background: `${watchedColor}08`, borderColor: `${watchedColor}22` }}
                      className="bg-gray-50 dark:bg-gray-800/50 border-gray-200 dark:border-gray-700">
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: `${watchedColor}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>
                        {watchedIcon}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 13, fontWeight: 700 }} className="text-gray-900 dark:text-white truncate">
                          {watch("title") || "Your Goal Name"}
                        </p>
                        <p style={{ fontSize: 11 }} className="text-gray-500 dark:text-gray-400">
                          {watch("target") ? fmtCurrency(Number(watch("target"))) : "₹0"} target
                        </p>
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, color: watchedColor }}>0%</span>
                    </div>
      ...
      <AnimatePresence>
        {progressGoal && (
          <ProgressModal
            ...
            onSubmit={amount => updateProgress.mutate({ id: progressGoal.id, amount })}
