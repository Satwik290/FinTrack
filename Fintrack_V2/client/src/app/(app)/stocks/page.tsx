...
const EXCHANGE_CONFIG = {
  NSE: {
    ...
    color: "#6366f1",
    emoji: "🇮🇳",
    hint: "e.g. TCS, INFY, RELIANCE",
  },
  US: {
    label: "US Market",
    color: "#f59e0b",
    emoji: "🇺🇸",
    hint: "e.g. AAPL, GOOGL, TSLA",
  },
  CRYPTO: {
    label: "Crypto",
    color: "#f97316",
    emoji: "₿",
    hint: "e.g. BTC-USD, ETH-USD",
  },
} as const;
...
function AddStockModal({ onClose }: { onClose: () => void }) {
  ...
  const { data: results = [], isLoading: searching } = useStockSearch(
    searchQ,
    exchange,
  );
    ...
    >
      <div className="modal-box" style={{ maxWidth: 500 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {(
            ...
            <button
              ...
              style={{
                flex: 1,
                background: exchange === key ? c.color : "var(--bg-surface-2)",
                color: exchange === key ? "#fff" : "var(--text-secondary)",
                border: "none",
                fontSize: 12,
              }}
            >
        ...
        >
          <div>
            ...
            <input
              ...
              type="number"
              step="0.0001"
              placeholder={exchange === "CRYPTO" ? "0.05" : "10"}
              value={form.quantity}
              onChange={(e) =>
        ...
        <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 6 }}>
          {exchange !== "NSE"
            ? "💡 Enter price in ₹ — we fetch live USD→INR rate for current value"
            : ""}
        </p>
...
export default function StocksPage() {
  ...
  const filtered = portfolio.holdings.filter(
    (h) => filterExchange === "ALL" || h.exchange === filterExchange,
  );

  // Allocation donut data
  const pieData = Object.entries(summary.byExchange).map(
    ([exchange, value]) => ({
      name:
        EXCHANGE_CONFIG[exchange as keyof typeof EXCHANGE_CONFIG]?.label ??
        exchange,
      value: Math.round(value),
      color:
        EXCHANGE_CONFIG[exchange as keyof typeof EXCHANGE_CONFIG]?.color ??
        "#94a3b8",
    }),
  );
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Yahoo Finance (15 min delay) · CoinGecko · All values in ₹
          </p>
        </div>
          ...
          {pieData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                ...
                <Tooltip
                  // Fix: Explicitly use 'any' and cast to 'number' to satisfy Recharts strict type check
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {(["ALL", "NSE", "US", "CRYPTO"] as const).map((e) => (
            ...
            style={{
              background:
                filterExchange === e
                  ? e === "ALL"
                    ? "var(--indigo-500)"
                    : EXCHANGE_CONFIG[e as keyof typeof EXCHANGE_CONFIG]?.color
                  : "var(--bg-surface-2)",
          ...
          >
            {e === "ALL"
              ? "All"
              : `${EXCHANGE_CONFIG[e as keyof typeof EXCHANGE_CONFIG].emoji} ${EXCHANGE_CONFIG[e as keyof typeof EXCHANGE_CONFIG].label}`}
          </button>
        ))}
      </div>
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        ...
        {!isLoading && !isError && filtered.length === 0 && (
          ...
          >
            ...
            <p style={{ fontSize: 13 }}>
              Add NSE stocks, US equities, or crypto to track your portfolio
            </p>
          </div>
        )}
        {!isLoading &&
          filtered.map((h) => (
            <HoldingRow
              key={h.id}
              h={h}
              onDelete={() => deleteStock.mutate(h.id)}
            />
          ))}
      </div>
