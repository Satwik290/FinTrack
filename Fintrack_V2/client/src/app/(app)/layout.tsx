...
import {
  ...
  ChevronLeft, ChevronRight, LogOut, X, Menu, Landmark,
} from "lucide-react";
  ...
  return (
    ...
    >
      <div style={{
        ...
        <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: "linear-gradient(135deg,#6366f1,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {open && (
        <>
          ...
          <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
            style={{ position: "fixed", left: 0, top: 0, bottom: 0, width: 280, background: "var(--bg-surface)", zIndex: 50, display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)" }}>
            ...
            <nav style={{ flex: 1, padding: "16px 12px", display: "flex", flexDirection: "column", gap: 4, overflowY: "auto" }}>
              {NAV.map(({ href, icon: Icon, label }) => (
                <Link key={href} href={href} className={`nav-item ${pathname.startsWith(href) ? "active" : ""}`} onClick={onClose} style={{ fontSize: 15, padding: "12px 16px" }}>
                  <Icon size={20} style={{ flexShrink: 0 }} /><span>{label}</span>
                </Link>
              ))}
            </nav>
...
function MobileTopBar({ onMenuOpen }: { onMenuOpen: () => void }) {
  ...
  return (
    <header style={{ height: "var(--mobile-topbar-height)", background: "var(--bg-surface)", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", padding: "0 16px", gap: 12, position: "sticky", top: 0, zIndex: 20 }}>
      <button onClick={onMenuOpen} className="btn btn-icon" style={{ width: 36, height: 36, flexShrink: 0 }}><Menu size={18} /></button>
      ...
      <button onClick={toggleDark} className="btn btn-icon" style={{ width: 36, height: 36 }}>{isDark ? "☀️" : "🌙"}</button>
      <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 12, flexShrink: 0 }}>{initials}</div>
    </header>
  );
}
...
function DesktopTopBar() {
  ...
  const initials = user?.email?.slice(0, 2).toUpperCase() ?? "FT";
  return (
    <header style={{ height: 64, flexShrink: 0, background: "var(--bg-surface)", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, position: "sticky", top: 0, zIndex: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto" }}>
        ...
        <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg,#6366f1,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", flexShrink: 0 }}>{initials}</div>
      </div>
    </header>
  );
}
  ...
  if (isMobile) {
    return (
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", background: "var(--bg-base)" }}>
        ...
        <main className="app-main" style={{ flex: 1, padding: "16px", paddingBottom: "calc(64px + 16px)", minWidth: 0 }}>{children}</main>
        <BottomNav />
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-base)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", marginLeft: sidebarW, transition: "margin-left 0.3s cubic-bezier(0.16,1,0.3,1)", minWidth: 0 }}>
        <DesktopTopBar />
        <main className="app-main" style={{ flex: 1, padding: "28px 32px", minWidth: 0 }}>{children}</main>
      </div>
    </div>
  );
}